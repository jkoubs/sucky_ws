...
class PathInterpolator(Node):
    def __init__(self):
        ...
        self.resolution = self.get_parameter('interpolation_resolution').get_parameter_value().double_value
        ...
        self.get_logger().info(f'Interpolating from "{input_topic}" to "{output_topic}" with resolution {self.resolution} meters.')
    ...
    def interpolate_path(self, msg):
        ...
        for i in range(len(msg.poses) - 1):
            ...
            steps = max(1, int(segment_length / self.resolution))
            
            for j in range(steps):
                t = j / steps
                interp_pose = PoseStamped()
                interp_pose.header = msg.header
                interp_pose.pose.position.x = start.pose.position.x + t * dx
                interp_pose.pose.position.y = start.pose.position.y + t * dy
                interp_pose.pose.position.z = start.pose.position.z + t * dz
