...
class InterpolatorNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('x_acc', 0.2)
        self.declare_parameter('yaw_vel', 0.1)
        self.declare_parameter('yaw_acc', 0.1)
        self.declare_parameter('resolution', 0.2)  # meters

        self.x_vel = self.get_parameter('x_vel').get_parameter_value().double_value
        self.x_acc = self.get_parameter('x_acc').get_parameter_value().double_value
        self.yaw_vel = self.get_parameter('yaw_vel').get_parameter_value().double_value
        self.yaw_acc = self.get_parameter('yaw_acc').get_parameter_value().double_value
        ...
        self.get_logger().info('InterpolatorNode is ready and waiting for /plan_smoothed...')

    def on_path_received(self, msg):
        if len(msg.poses) < 2:
            self.get_logger().warn('Path has fewer than 2 poses. Skipping.')
            return
        ...
        for i in range(len(msg.poses) - 1):
            ...
            interp = SectionInterpolator(from_pose, to_pose, self.x_vel, self.x_acc, self.yaw_vel, self.yaw_acc)
            poses = interp.generate_interpolated_poses(self.resolution)
        ...
        # Last Pose data
        last_pose = interpolated_path.poses[-1].pose
        ...
        self.get_logger().info(
            f'Last pose: x={last_pose.position.x:.2f}, y={last_pose.position.y:.2f}, yaw={yaw:.2f} rad'
        )
        self.get_logger().info(f'Frame of final pose: {interpolated_path.header.frame_id}')
...
class SectionInterpolator:
    def __init__(self, from_pose, to_pose, x_vel, x_acc, yaw_vel, yaw_acc):
